...
- (void)closeOpenResultSets {
    ...
    while((returnedResultSet = [e nextObject])) {
        SRCResultSet *rs = (SRCResultSet *)[returnedResultSet pointerValue];
...
- (void)resultSetDidClose:(SRCResultSet *)resultSet {
    NSValue *setValue = [NSValue valueWithNonretainedObject:resultSet];
    [openResultSets removeObject:setValue];
}

- (SRCStatement*)cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(SRCStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    SRCResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The SRCDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (crashOnErrors) {
        NSAssert1(false, @"The SRCDatabase %@ is currently in use.", self);
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!db) {
        ...
        NSLog(@"The SRCDatabase %@ is not open.", self);
        
    #ifndef NS_BLOCK_ASSERTIONS
        if (crashOnErrors) {
            NSAssert1(false, @"The SRCDatabase %@ is not open.", self);
        }
    #endif
...
- (SRCResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (!statement) {
        statement = [[SRCStatement alloc] init];
    ...
    rs = [SRCResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (SRCResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (SRCResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (SRCResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[SRCStatement alloc] init];
