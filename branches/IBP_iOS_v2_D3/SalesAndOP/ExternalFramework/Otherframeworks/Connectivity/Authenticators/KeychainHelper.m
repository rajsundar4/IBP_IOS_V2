...
static NSString * const kCredentialsIdentifier = @"UserCredentials";
static NSString * const kClientCertificateIdentifier = @"ClientCertificate";
    ...
    if (status == errSecSuccess)
    {
        ...
        if (status == errSecSuccess) {
            ...
            if (error) {
                ...
                *error = [NSError errorWithDomain:ERROR_DOMAIN code:status userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
            }
        }
...
+ (BOOL)saveIdentityToKeychain:(SecIdentityRef)anIdentity error:(NSError * __autoreleasing *)error
{
    ...
    if (anIdentity == NULL) {
        if (error) {
            ...
            *error = [NSError errorWithDomain:ERROR_DOMAIN code:errSecParam userInfo:@{NSLocalizedDescriptionKey: errorMessage}];
        }
        return NO;
    }
